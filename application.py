...
def inject_css() -> None:
    ...
    /* Model comparison extras */
    .winner-glow{box-shadow:0 0 0 3px rgba(139,92,246,0.25),0 8px 24px rgba(139,92,246,0.15);}
    .carrier-card{background:white;border:1px solid #e5e7eb;border-radius:14px;padding:16px;
        box-shadow:0 4px 16px rgba(0,0,0,0.06);transition:all .2s;}
    .carrier-card:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(0,0,0,0.10);}
    </style>
...
def _make_models() -> dict:
    return {
        ...
        "GradBoost": GradientBoostingRegressor(
            ...
            learning_rate=LEARNING_RATE_GB, subsample=SUBSAMPLE_GB, random_state=42,
        )
    }
...
def render_model_quality(res: dict) -> None:
    ...
    kpi(c5, "Accuracy", f"{max(0.0,round((1-res['nrmse'])*100,1)):.1f}%", "mint", "1 − NRMSE")
    sp(0.5)
    st.markdown(
...
def build_sku_production_plan(n_future: int = N_FUTURE_MONTHS) -> pd.DataFrame:
    ...
    for cat, grp in needs.groupby("Category"):
        cat_wh = (wh_cat[wh_cat["Category"] == cat].sort_values("wh_share", ascending=False).reset_index(drop=True))
...
def build_context(n_future: int = N_FUTURE_MONTHS) -> str:
    ...
    return (
        ...
        f"Revenue by Category: {cat_str}\nRevenue by Channel: {ch_str}\n"
        f"Top Revenue Regions: {reg_str}\nOrder Volume by Region: {reg_vol_str}\n"
...
def call_llm(messages: list, system: str, api_key: str) -> str:
    ...
    body = {
        ...
        "temperature": LLM_TEMP, "messages": [{"role": "system", "content": system}] + messages,
    }
    try:
...
# ─────────────────────────────────────────────────────────────────────────────
#  MODEL COMPARISON PAGE  (fully inline — no separate file needed)
# ─────────────────────────────────────────────────────────────────────────────
def _run_all_models_on_series(vals: np.ndarray, ds_idx, n_future: int) -> dict | None:
    n = len(vals)
    if n < MIN_HISTORY_MONTHS:
        return None
    regime_idx = _detect_regime(vals)
    ...
    X_fut  = X_all[n:]
    h = min(4, n // 3)
    X_tr, y_tr = X_hist[:-h], vals[:-h]
    X_te, y_te = X_hist[-h:],  vals[-h:]
    models = _make_models()
    fitted_pm:   dict[str, np.ndarray] = {}
    forecast_pm: dict[str, np.ndarray] = {}
    eval_pm:     dict[str, np.ndarray] = {}
    metrics_pm:  dict[str, dict]       = {}
    for mname, mdl in models.items():
        pipe_full = Pipeline([("sc", StandardScaler()), ("m", mdl)])
        pipe_full.fit(X_hist, vals)
        fitted_pm[mname]   = np.maximum(pipe_full.predict(X_hist), 0)
        forecast_pm[mname] = np.maximum(pipe_full.predict(X_fut),  0)
        mdl2 = _make_models()[mname]
        pipe_eval = Pipeline([("sc", StandardScaler()), ("m", mdl2)])
        pipe_eval.fit(X_tr, y_tr)
        y_pred = np.maximum(pipe_eval.predict(X_te), 0)
        eval_pm[mname] = y_pred
        rmse  = float(np.sqrt(mean_squared_error(y_te, y_pred)))
        mae   = float(mean_absolute_error(y_te, y_pred))
        my    = float(np.mean(y_te)) if float(np.mean(y_te)) > 0 else 1.0
        nrmse = rmse / my
        ss_r  = float(np.sum((y_te - y_pred)**2))
        ss_t  = float(np.sum((y_te - np.mean(y_te))**2))
        r2    = float(max(0.0, 1 - ss_r / (ss_t + 1e-9)))
        mape  = float(np.mean(np.abs((y_te - y_pred) / np.maximum(np.abs(y_te), 1e-9))) * 100)
        metrics_pm[mname] = dict(rmse=rmse, mae=mae, nrmse=nrmse, r2=r2, mape=mape)
    inv_rmse = {m: 1.0 / (metrics_pm[m]["rmse"] + 1e-9) for m in models}
    tot_w    = sum(inv_rmse.values())
    weights  = {m: v / tot_w for m, v in inv_rmse.items()}
    ens_eval     = sum(weights[m] * eval_pm[m]     for m in models)
    ens_fitted   = sum(weights[m] * fitted_pm[m]   for m in models)
    ens_forecast = sum(weights[m] * forecast_pm[m] for m in models)
    ens_rmse  = float(np.sqrt(mean_squared_error(y_te, ens_eval)))
    ens_mae   = float(mean_absolute_error(y_te, ens_eval))
    my        = float(np.mean(y_te)) if float(np.mean(y_te)) > 0 else 1.0
    ens_nrmse = ens_rmse / my
    ss_r  = float(np.sum((y_te - ens_eval)**2))
    ss_t  = float(np.sum((y_te - np.mean(y_te))**2))
    ens_r2    = float(max(0.0, 1 - ss_r / (ss_t + 1e-9)))
    ens_mape  = float(np.mean(np.abs((y_te - ens_eval) / np.maximum(np.abs(y_te), 1e-9))) * 100)
    metrics_pm["Ensemble"] = dict(rmse=ens_rmse, mae=ens_mae, nrmse=ens_nrmse, r2=ens_r2, mape=ens_mape)
    ts_idx    = _to_ts(ds_idx)
    last_dt   = ts_idx[-1]
    fut_dates = pd.date_range(last_dt + pd.offsets.MonthBegin(1), periods=n_future, freq="MS")
    residuals_ens = vals - ens_fitted
    resid_std     = float(residuals_ens.std())
    log_std = np.log1p(resid_std / (float(np.mean(vals)) + 1e-9))
    steps   = np.arange(1, n_future + 1)
    ci_lo   = np.maximum(ens_forecast * np.exp(-1.645 * log_std * np.sqrt(steps)), 0)
    ci_hi   = ens_forecast * np.exp(1.645 * log_std * np.sqrt(steps))
    return dict(
        hist_ds=ts_idx, hist_y=vals, eval_ds=ts_idx[-h:], eval_y=y_te,
        fitted_pm=fitted_pm, forecast_pm=forecast_pm, eval_pm=eval_pm,
        ens_fitted=ens_fitted, ens_forecast=ens_forecast,
        ci_lo=ci_lo, ci_hi=ci_hi, fut_ds=fut_dates,
        metrics=metrics_pm, weights=weights,
    )

def _mc_grade(nrmse: float, r2: float) -> tuple:
    if   nrmse < 0.10 and r2 >= 0.95: return "A+", "#22c55e"
    elif nrmse < 0.15 and r2 >= 0.90: return "A",  "#86efac"
    elif nrmse < 0.20 and r2 >= 0.85: return "B+", "#3b82f6"
    elif nrmse < 0.25 and r2 >= 0.75: return "B",  "#93c5fd"
    elif nrmse < 0.35 and r2 >= 0.60: return "C",  "#fbbf24"
...
def page_model_comparison() -> None:
    ...
    st.markdown(
        ...
        "<b>Inverse-RMSE Ensemble</b> on every metric series. "
        "Best model per series is chosen by lowest NRMSE on the hold-out window."
        "</div>", unsafe_allow_html=True)
    ...
    chosen = s1.multiselect("Select series to benchmark",
        list(SERIES_OPTIONS.keys()),
        default=["Overall Orders", "Overall Quantity", "Overall Revenue"],
        key="mc_series")
    show_charts = s2.toggle("Show forecast charts", value=True, key="mc_charts")
    ...
    MC = MODEL_COLORS  # already defined at top
    ...
    avg_nrmse  = df_summ.groupby("Model")["NRMSE %"].mean().reindex(MODEL_ORDER)
    avg_r2     = df_summ.groupby("Model")["R²"].mean().reindex(MODEL_ORDER)
    overall_winner = win_counts.idxmax()
    total_series   = len([s for s in all_results.values() if s is not None])

    lc = st.columns(4)
    for col, mname in zip(lc, MODEL_ORDER):
        wins  = int(win_counts.get(mname, 0))
        nrmse = float(avg_nrmse.get(mname, 0))
        r2    = float(avg_r2.get(mname, 0))
    ...
    with bc1:
        ...
        fig_nrmse.update_layout(**CD(), height=310, barmode="group",
            xaxis={**gX(),"tickangle":-30,"tickfont":dict(size=9)},
            yaxis={**gY(),"title":"NRMSE %"},
            legend={**leg(),"orientation":"h","y":-0.38},
    ...
    with bc2:
        ...
        for mname in MODEL_ORDER:
            ...
            fig_r2.add_trace(go.Bar(name=mname, x=pivot_r2.index, y=pivot_r2[mname],
                marker=dict(color=MC[mname], opacity=0.85, line=dict(color="rgba(0,0,0,0)")),
                text=[f"{v:.3f}" for v in pivot_r2[mname]], textposition="outside",
                textfont=dict(color="#334155", size=8)))
        fig_r2.add_hline(y=0.90, line_dash="dash", line_color="#22c55e",
            annotation_text=" R²=0.90 target", annotation_font=dict(color="#22c55e", size=9))
        fig_r2.update_layout(**CD(), height=310, barmode="group",
            xaxis={**gX(),"tickangle":-30,"tickfont":dict(size=9)},
            yaxis={**gY(),"title":"R² Score","range":[0,1.12]},
            legend={**leg(),"orientation":"h","y":-0.38},
            title=dict(text="R² Score (higher = better)", font=dict(size=11, color="#64748b")))
        st.plotly_chart(fig_r2, use_container_width=True, key="mc_r2")

    sp()

    # ── Radar chart ───────────────────────────────────────────────────────
    sec("🕸️ Multi-Metric Radar — Normalised 0–1 Profile")
    radar_rows = []
    for mname in MODEL_ORDER:
    ...
    for _, row in radar_df.iterrows():
        mname  = row["Model"]
        vals_r = [float(row[c]) for c in radar_cats]
    ...
    res = all_results.get(chosen_deep)
    if res is None:
        st.warning("Insufficient data.")
        return

    best_m_deep = min([m for m in MODEL_ORDER if m in res["metrics"]], key=lambda m: res["metrics"][m]["nrmse"])

    m_best = res["metrics"][best_m_deep]
    st.markdown(
    ...
    deep_rows = []
    for mname in MODEL_ORDER:
        if mname not in res["metrics"]: continue
        m = res["metrics"][mname]
        ...
        deep_rows.append({
            ...
            "NRMSE %": round(m["nrmse"]*100,2), "MAPE %": round(m["mape"],2),
            "R²": round(m["r2"],4), "Grade": grade,
            "Winner": "✅" if mname == best_m_deep else "",
        })
    ...
    if show_charts:
        ...
        fig_fc.add_vline(x=res["fut_ds"][0], line_dash="dash",
            line_color="rgba(139,92,246,0.5)", line_width=1.5)
        fig_fc.add_trace(go.Scatter(x=res["hist_ds"], y=res["hist_y"], name="Actual",
            line=dict(color="#4a5e7a", width=2.2),
            hovertemplate="<b>%{x|%b %Y}</b><br>%{y:,.0f}<extra></extra>"))
        for mname, clr, dash in MODEL_STYLES:
            if mname in res["fitted_pm"]:
                is_best = (mname == best_m_deep)
                fig_fc.add_trace(go.Scatter(
                    x=res["hist_ds"], y=res["fitted_pm"][mname],
                    name=f"{mname} fit", line=dict(color=clr, width=1.8 if is_best else 1.2, dash=dash),
                    opacity=0.7 if is_best else 0.45))
        fig_fc.add_trace(go.Scatter(x=res["hist_ds"], y=res["ens_fitted"], name="Ensemble fit",
            line=dict(color="#8B5CF6", width=1.8, dash="dot"), opacity=0.65))
        for mname, clr, dash in MODEL_STYLES:
            if mname in res["forecast_pm"]:
                is_best = (mname == best_m_deep)
                fig_fc.add_trace(go.Scatter(
                    x=res["fut_ds"], y=res["forecast_pm"][mname],
                    name=f"{mname} fc" + (" 🏆" if is_best else ""),
                    line=dict(color=clr, width=3 if is_best else 1.5, dash=dash),
                    mode="lines+markers",
                    marker=dict(size=9 if is_best else 5, color=clr, line=dict(color="#fff", width=2))))
        fig_fc.add_trace(go.Scatter(x=res["fut_ds"], y=res["ens_forecast"],
        ...
        fig_fc.add_trace(go.Scatter(x=res["eval_ds"], y=res["eval_y"], name="Hold-out actuals",
            mode="markers",
        ...
        residuals = res["hist_y"] - res["ens_fitted"]
        with rc1:
            fig_res = go.Figure()
            fig_res.add_trace(go.Scatter(x=list(res["hist_ds"]), y=list(residuals),
        ...
        with rc2:
            fig_hist = go.Figure(go.Histogram(x=residuals, nbinsx=12,
                marker=dict(color="#8B5CF6", opacity=0.75, line=dict(color="#fff", width=1))))
            fig_hist.add_vline(x=0, line_dash="dash", line_color="rgba(0,0,0,0.3)")
            fig_hist.update_layout(**CD(), height=220,
                xaxis={**gX(),"title":"Residual value"}, yaxis={**gY(),"title":"Count"},
                title=dict(text="Residual Distribution", font=dict(size=11, color="#64748b")))
            st.plotly_chart(fig_hist, use_container_width=True, key="mc_res_hist")
    ...
    avn    = float(avg_nrmse.get(overall_winner, 0))
    avr    = float(avg_r2.get(overall_winner, 0))
    ...
    st.markdown(
        f"""<div style='background:linear-gradient(135deg,#fdf4ff,#fff);
          ...
          <div style='margin-top:14px;font-size:12px;color:#475569;line-height:1.7;border-top:1px solid #e5e7eb;padding-top:12px'>
            ...
            Even when a base model wins individual series, the <b>Ensemble</b> is recommended for
            production — its inverse-RMSE blending consistently out-performs any single model on
            average and its confidence intervals capture forecast uncertainty across all horizon lengths.
          </div>
        </div>""", unsafe_allow_html=True)
...
def page_overview() -> None:
    ...
    st.markdown("""
     <div style='background:linear-gradient(135deg,#0f172a,#1e3a8a,#2563eb);border-radius:18px;
      ...
      <div style='font-size:11px;font-family:DM Mono,monospace;color:#93c5fd;letter-spacing:.14em;
           text-transform:uppercase;margin-top:6px'>AI Driven Demand to Delivery Optimization System</div>
     </div>""", unsafe_allow_html=True)
    ...
    <div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px'>
      <div class='card'><div style='font-size:12px;font-weight:800;color:#1e3a8a;margin-bottom:8px'>Orders & Revenue</div>
        <div style='font-size:12px;line-height:1.8;color:#475569'><b>5,010 orders</b> · Jan 2024 – Dec 2025<br>73.9% Delivered · 12.3% Shipped<br>9.3% Returned · 4.5% Cancelled<br>Avg order value: <b>₹8,159</b></div></div>
      <div class='card'><div style='font-size:12px;font-weight:800;color:#1e3a8a;margin-bottom:8px'>Sales Channels</div>
        <div style='font-size:12px;line-height:1.8;color:#475569'><b>Amazon.in</b> — 2,099 orders (41.9%)<br><b>Shiprocket</b> — 1,761 orders (35.1%)<br><b>INCREFF B2B</b> — 1,150 orders (23.0%)<br>Multi-channel D2C + B2B mix</div></div>
      <div class='card'><div style='font-size:12px;font-weight:800;color:#1e3a8a;margin-bottom:8px'>Geography</div>
        <div style='font-size:12px;line-height:1.8;color:#475569'><b>9 Indian regions:</b><br>Maharashtra · Delhi · Uttar Pradesh<br>Karnataka · Gujarat · Tamil Nadu<br>Telangana · West Bengal · Rajasthan</div></div>
      <div class='card'><div style='font-size:12px;font-weight:800;color:#1e3a8a;margin-bottom:8px'>Operations</div>
        <div style='font-size:12px;line-height:1.8;color:#475569'><b>4 Warehouses:</b> Delhi · Mumbai · Bengaluru · Hyderabad<br><b>5 Carriers:</b> BlueDart · Delhivery · DTDC · Ecom Express · XpressBees<br>Avg delivery: <b>2.2 days</b></div></div>
      <div class='card'><div style='font-size:12px;font-weight:800;color:#1e3a8a;margin-bottom:8px'>Products</div>
        <div style='font-size:12px;line-height:1.8;color:#475569'><b>50 SKUs</b> across 4 categories:<br>Electronics & Mobiles (dominant)<br>Fashion & Apparel · Home & Kitchen<br>Health & Personal Care</div></div>
    </div></div>""", unsafe_allow_html=True)
    st.markdown("""
    <div class='about-section'>
    <div style='font-size:16px;font-weight:900;margin-bottom:6px'>Analytics Pipeline</div>
    <div style='font-size:12px;color:#64748b;margin-bottom:14px'>Modules are chained: demand forecast → inventory → production → logistics routing.</div>
    <div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px'>
      <div class='card' style='border-top:3px solid #3b82f6'><div style='font-size:11px;font-weight:800;color:#3b82f6;letter-spacing:.06em;text-transform:uppercase'>1 · Demand Forecasting</div>
        <div style='font-size:11px;font-weight:700;color:#0f172a;margin:6px 0 4px'><i>How much will sell?</i></div>
        <div style='font-size:11.5px;color:#475569;line-height:1.7'>Ridge + Random Forest + Gradient Boosting <b>ensemble</b> forecasts orders, quantity and revenue.</div></div>
      <div class='card' style='border-top:3px solid #f59e0b'><div style='font-size:11px;font-weight:800;color:#f59e0b;letter-spacing:.06em;text-transform:uppercase'>2 · Inventory Optimisation</div>
        <div style='font-size:11px;font-weight:700;color:#0f172a;margin:6px 0 4px'><i>Which SKUs need restocking?</i></div>
        <div style='font-size:11.5px;color:#475569;line-height:1.7'>Wilson EOQ, Safety Stock, ROP and ABC classification. Demand window matches forecast horizon.</div></div>
      <div class='card' style='border-top:3px solid #8b5cf6'><div style='font-size:11px;font-weight:800;color:#8b5cf6;letter-spacing:.06em;text-transform:uppercase'>3 · Production Planning</div>
        <div style='font-size:11px;font-weight:700;color:#0f172a;margin:6px 0 4px'><i>How many units to make, when?</i></div>
        <div style='font-size:11.5px;color:#475569;line-height:1.7'>Monthly production targets from forecasted demand. Critical SKUs get urgency boosts in Month 1–2.</div></div>
      <div class='card' style='border-top:3px solid #059669'><div style='font-size:11px;font-weight:800;color:#059669;letter-spacing:.06em;text-transform:uppercase'>4 · Logistics Optimisation</div>
        <div style='font-size:11px;font-weight:700;color:#0f172a;margin:6px 0 4px'><i>Which carrier, at what cost?</i></div>
        <div style='font-size:11.5px;color:#475569;line-height:1.7'>Carrier composite score = weighted(speed + cost + return rate). Identifies cheapest carrier per region.</div></div>
      <div class='card' style='border-top:3px solid #7c3aed'><div style='font-size:11px;font-weight:800;color:#7c3aed;letter-spacing:.06em;text-transform:uppercase'>5 · Model Comparison</div>
        <div style='font-size:11px;font-weight:700;color:#0f172a;margin:6px 0 4px'><i>Which algorithm wins?</i></div>
        <div style='font-size:11.5px;color:#475569;line-height:1.7'>Full benchmark of Ridge, RF, GradBoost and Ensemble across all metric series with grade, radar and residual plots.</div></div>
      <div class='card' style='border-top:3px solid #ef4444'><div style='font-size:11px;font-weight:800;color:#ef4444;letter-spacing:.06em;text-transform:uppercase'>6 · Decision Intelligence</div>
        <div style='font-size:11px;font-weight:700;color:#0f172a;margin:6px 0 4px'><i>What action should I take?</i></div>
        <div style='font-size:11.5px;color:#475569;line-height:1.7'>LLM (Llama 3.3-70B via Groq) answers natural language questions with specific SKU names, cost figures and day counts.</div></div>
    </div></div>""", unsafe_allow_html=True)
...
def page_demand() -> None:
    ...
    for cat in cat_monthly.columns:
        ...
        if r: proj_next[cat] = float(r["forecast"].sum())
    if 2024 in yr_rev.index and 2025 in yr_rev.index:
        ...
        for cat in yr_rev.columns:
            ...
            rows.append({
                "Category": cat, "2024 ₹M": round(r24/1e6,1), "2025 ₹M": round(r25/1e6,1),
...
def page_inventory() -> None:
    ...
    with tab_alerts:
        ...
        if sv.empty:
            ...
            STATUS_CLR = {"🔴 Critical":"#ef4444","🟡 Low":"#f59e0b","🟢 Adequate":"#22c55e"}
            fig_sc = go.Figure()
            ax_max = max(sv["Current_Stock"].max(), sv["ROP"].max()) * 1.1
            fig_sc.add_trace(go.Scatter(x=[0, ax_max], y=[0, ax_max], mode="lines",
            ...
            for status, clr in STATUS_CLR.items():
                ...
                fig_sc.add_trace(go.Scatter(x=grp["Current_Stock"], y=grp["ROP"],
                    mode="markers", name=status,
                    marker=dict(size=bubble_sz, color=clr, opacity=0.82,
                        line=dict(color="#FFFFFF", width=1.5), sizemode="area",
                        sizeref=2.0*60/(40.0**2), sizemin=6),
                    customdata=grp[["Product_Name","SKU_ID","Prod_Need","Demand_6M","Demand_Cover_Pct"]].values,
                    hovertemplate=(
                        "<b>%{customdata[0]}</b><br>SKU: %{customdata[1]}<br>Stock: %{x}<br>"
                        f"ROP: %{{y}}<br>{n_future}M Demand: %{{customdata[3]:,}} units<br>"
                        "Stock Covers: %{customdata[4]:.0f}% of demand<br>Produce: <b>%{customdata[2]} units</b>")))
            fig_sc.update_layout(**CD(), height=400,
            ...
            if not action.empty:
                ...
                tbl = action[["SKU_ID","Product_Name","Category","ABC","Status",
                    "Current_Stock","Demand_6M","Demand_Cover_Pct","ROP","EOQ","SS","Prod_Need"]].copy()
                tbl.columns = ["SKU","Product","Category","ABC","Status",
                    "Stock",f"{n_future}M Demand","Covers %","ROP","EOQ","Safety Stock","Units to Produce"]
...
# ─────────────────────────────────────────────────────────────────────────────
#  PAGE: PRODUCTION  (unchanged from original)
# ─────────────────────────────────────────────────────────────────────────────
def page_production() -> None:
    ...
    d3    = d2[["Month","Category","Current_Stock","Demand_6M_Cat","Prod_Need_Cat",
        "Demand_Forecast","Crit_Boost","Low_Boost","Production","CI_Lo","CI_Hi"]].copy()
    d3.columns = ["Month","Category","Cat Stock",f"{n_future}M Demand","Inv Prod Need",
        "Demand Fc","Crit Boost","Low Boost","Production","Demand Lo","Demand Hi"]
    st.dataframe(d3.sort_values("Month"), use_container_width=True, hide_index=True)
    if cat_f != "All":
        banner(f"<b>Filtered subtotal — {cat_f}:</b> Production = <b>{int(d2['Production'].sum()):,} units</b>"
               f" | Demand Forecast = <b>{int(d2['Demand_Forecast'].sum()):,} units</b>", "sky")
    sp()
    st.markdown("<div style='font-size:22px;font-weight:900;color:black'>Fulfillment & Routing Plan</div>",
        unsafe_allow_html=True)
    sku_plan = build_sku_production_plan(n_future)
    if sku_plan.empty:
    ...
    with pt2:
        sec("Warehouse Routing Plan")
        routing_tbl = sku_plan[["Target_Warehouse","SKU_ID","Product_Name","Category","ABC","Urgency",
            "Prod_Need","Days_Left","Est_Ship_Cost","WH_Share_Pct"]].copy()
    ...
    with pt3:
        ...
        urg_color_map = {"🔴 Urgent":"#ef4444","🟠 High":"#f97316","🟡 Medium":"#eab308","🟢 Normal":"#22c55e"}
        with va1:
            urg_counts = sku_plan["Urgency"].value_counts().reset_index()
            urg_counts.columns = ["Urgency","Count"]
            fig_d = go.Figure(go.Pie(labels=urg_counts["Urgency"], values=urg_counts["Count"], hole=0.55,
        ...
        with va2:
            ...
            for urg, clr in urg_color_map.items():
                ...
                if sub.empty: continue
                fig_bu.add_trace(go.Bar(name=urg, x=sub["Category"], y=sub["Prod_Need"],
                    marker=dict(color=clr, line=dict(color="rgba(0,0,0,0)")),
                    text=sub["Prod_Need"].astype(int), textposition="inside",
                    textfont=dict(color="white", size=9)))
            fig_bu.update_layout(**CD(), height=260, barmode="stack",
                xaxis={**gX(),"tickangle":-10}, yaxis={**gY(),"title":"Units to Produce"},
                legend={**leg(),"orientation":"h","y":-0.32},
        ...
        fig_hl = go.Figure(go.Bar(
            x=top20_s["Days_Left"].clip(upper=60), y=top20_s["Label"], orientation="h",
            ...
            hovertemplate="<b>%{y}</b><br>Days left: %{x:.0f}d<br>Category: %{customdata[0]}<br>Warehouse: %{customdata[1]}<br>Urgency: %{customdata[2]}<extra></extra>"))
...
# ─────────────────────────────────────────────────────────────────────────────
#  PAGE: LOGISTICS  (improved heatmap + best carrier)
# ─────────────────────────────────────────────────────────────────────────────
def page_logistics() -> None:
    ...
    with t1:
        ...
        for i, (_, r) in enumerate(carr.iterrows()):
            fig.add_trace(go.Scatter(
                ...
                hovertemplate=(
                    ...
                    f"Return Rate: {r['Return_Rate']*100:.1f}%<br>Score: {r['Perf_Score']:.3f}<extra></extra>")))
        fig.update_layout(**CD(), height=280, showlegend=False,
        ...
        # -- Carrier metrics + IMPROVED best carrier per category
        ta1, ta2 = st.columns(2, gap="large")
        with ta1:
            sec("Carrier Metrics Table")
            d2 = carr[["Courier_Partner","Orders","Avg_Days","Avg_Cost","Return_Rate","Perf_Score","Delay_Index"]].copy()
            ...
            d2.columns = ["Carrier","Orders","Avg Days","Avg Cost ₹","Return Rate","Score","Delay Index"]
            st.dataframe(d2.sort_values("Score", ascending=False), use_container_width=True, hide_index=True)

            # Composite score horizontal bar
            carr_sorted = carr.sort_values("Perf_Score", ascending=True)
            fig_score = go.Figure(go.Bar(
                x=carr_sorted["Perf_Score"], y=carr_sorted["Courier_Partner"],
                orientation="h",
                marker=dict(
                    color=carr_sorted["Perf_Score"],
                    colorscale=[[0,"#fecaca"],[0.4,"#fde68a"],[0.7,"#bbf7d0"],[1,"#22c55e"]],
                    line=dict(color="rgba(0,0,0,0)"), showscale=False),
                text=[f"{v:.3f}" for v in carr_sorted["Perf_Score"]],
                textposition="outside", textfont=dict(color="#334155", size=10)))
            fig_score.update_layout(**CD(), height=180,
                xaxis={**gX(),"title":"Composite Score (higher = better)","range":[0,1]},
                yaxis=dict(showgrid=False, color="#64748b", automargin=True),
                title=dict(text="Overall Carrier Score Ranking", font=dict(size=11, color="#64748b")))
            st.plotly_chart(fig_score, use_container_width=True, key="log_score_bar")

        # ── IMPROVED Best Carrier per Category ────────────────────────────
        with ta2:
            sec("Best Carrier per Category — Detailed Breakdown")
            if not plan.empty:
                ...
                categories = sorted(cat_carr["Category"].unique())
                for cat in categories:
                    cat_sub = cat_carr[cat_carr["Category"] == cat].sort_values("Score", ascending=False)
                    best    = cat_sub.iloc[0]
                    worst   = cat_sub.iloc[-1]
                    planned_units = int(prod_by_cat.loc[prod_by_cat["Category"]==cat,"Production"].values[0]) \
                                    if cat in prod_by_cat["Category"].values else 0
                    rank_html = ""
                    for rank_i, (_, row_r) in enumerate(cat_sub.iterrows()):
                        bar_w = int(row_r["Score"] * 100)
                        medal = ["🥇","🥈","🥉","4️⃣","5️⃣"][min(rank_i, 4)]
                        rank_html += (
                            f"<div style='display:flex;align-items:center;gap:8px;margin:4px 0'>"
                            f"<span style='font-size:13px'>{medal}</span>"
                            f"<span style='font-size:11px;font-weight:700;color:#0f172a;min-width:90px'>{row_r['Courier_Partner']}</span>"
                            f"<div style='flex:1;background:#f1f5f9;border-radius:6px;height:8px;overflow:hidden'>"
                            f"<div style='width:{bar_w}%;height:100%;background:{"#22c55e" if rank_i==0 else "#94a3b8"};border-radius:6px'></div></div>"
                            f"<span style='font-size:10px;color:#64748b;font-family:DM Mono;min-width:36px'>{row_r['Score']:.3f}</span>"
                            f"<span style='font-size:9px;color:#94a3b8'>{row_r['Avg_Days']:.1f}d · ₹{row_r['Avg_Cost']:.0f}</span>"
                            f"</div>")
                    st.markdown(
                        f"""<div class='carrier-card' style='margin-bottom:10px;border-left:4px solid #22c55e'>
                          <div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px'>
                            <div>
                              <div style='font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:#64748b;font-family:DM Mono'>{cat}</div>
                              <div style='font-size:16px;font-weight:900;color:#0f172a'>🥇 {best["Courier_Partner"]}</div>
                              <div style='font-size:10px;color:#64748b;margin-top:2px'>
                                Score <b style='color:#22c55e'>{best["Score"]:.3f}</b> ·
                                {best["Avg_Days"]:.1f}d avg ·
                                ₹{best["Avg_Cost"]:.0f}/order ·
                                {best["Return_Rate"]*100:.1f}% returns
                              </div>
                            </div>
                            <div style='text-align:right'>
                              <div style='font-size:9px;color:#94a3b8;font-family:DM Mono'>PLANNED UNITS</div>
                              <div style='font-size:18px;font-weight:800;color:#1e3a8a'>{planned_units:,}</div>
                            </div>
                          </div>
                          <div style='font-size:9px;color:#94a3b8;font-family:DM Mono;text-transform:uppercase;margin-bottom:4px'>All carriers ranked</div>
                          {rank_html}
                        </div>""", unsafe_allow_html=True)
            else:
    ...
    with t2:
        ...
        sp(0.5)
        delay_thr_t2 = st.session_state.get("log_thr", DEFAULT_LEAD_TIME)
        ...
        with tb1:
            ...
            fig_cost = go.Figure()
            fig_cost.add_trace(go.Bar(name="Current ₹/order", x=opt["Region"], y=opt["Current_Avg_Cost"],
            ...
            fig_cost.add_trace(go.Bar(name="Optimal ₹/order", x=opt["Region"], y=opt["Min_Avg_Cost"],
                marker=dict(color="#22C55E", line=dict(color="rgba(0,0,0,0)")),
                text=[f"₹{v:.0f}" for v in opt["Min_Avg_Cost"]],
                textposition="outside", textfont=dict(color="#334155")))
            fig_cost.update_layout(**CD(), height=280, barmode="group",
                xaxis={**gX(),"tickangle":-30}, yaxis={**gY(),"title":"Avg Cost per Order ₹"},
        ...
        with tb2:
            ...
            fig_r = go.Figure(go.Bar(x=rd_s["Rate"], y=rd_s["Region"], orientation="h",
                ...
                textfont=dict(color="#334155")))
            fig_r.update_layout(**CD(), height=280,
    ...
    with t3:
        if fwd_plan.empty:
            st.info("No forward plan available.")
        else:
            fwd_agg = (fwd_plan.groupby("Month_dt").agg(
                Month=("Month","first"), Total_Units=("Prod_Units","sum"),
                Total_Orders=("Proj_Orders","sum"), Total_Ship_Cost=("Proj_Ship_Cost","sum"),
                CI_Lo=("CI_Lo_Units","sum"), CI_Hi=("CI_Hi_Units","sum"),
            ...
            with tc1:
                ...
                fig_fwd.add_trace(go.Scatter(x=x_ci, y=y_ci, fill="toself",
                    fillcolor="rgba(59,130,246,0.08)", line=dict(color="rgba(0,0,0,0)"),
                    name="Demand 90% CI", hoverinfo="skip"))
                fig_fwd.add_trace(go.Bar(x=fwd_agg["Month_dt"], y=fwd_agg["Total_Units"],
                    name="Planned Units",
                ...
                fig_fwd.update_layout(**CD(), height=260, barmode="overlay", xaxis=gX(),
                    yaxis={**gY(),"title":"Units"}, legend={**leg(),"orientation":"h","y":-0.28})
            ...
            inb_rows = [
                {"Month":row["Month"],"Month_dt":row["Month_dt"],"Warehouse":wh,
...
def page_chatbot() -> None:
    ...
    with st.sidebar:
        st.markdown("""<div style='margin-top:14px;border-top:1px solid rgba(255,255,255,0.08);
            ...
            unsafe_allow_html=True)
        api_key = st.text_input("Groq API Key", type="password", placeholder="gsk_xxxxxxxxxxxxxxxxx")
    ...
    system = (
        ...
        f"The active forecast horizon is {n_future} months.\n\n"
        "CROSS-MODULE REASONING (MANDATORY):\n"
        "- Every answer must trace the full supply chain: Demand → Inventory gap → Production → Logistics\n"
        "- 5–8 ▸ bullet points with exact numbers (SKU names, ₹ values, day counts)\n"
    ...
    for msg in st.session_state.chat_msgs:
        ...
        if role == "user":
            ...
            st.markdown(f"<div style='margin:10px 0'><div class='chat-ai-bubble'>{''.join(parts)}</div></div>",
                unsafe_allow_html=True)
    sp()
    ...
    with cb:
        if st.button("Send", use_container_width=True):
            if not key_ok: st.warning("Enter your API key first.")
            elif user_in.strip():
...
def main() -> None:
    ...
    horizon_val = st.sidebar.select_slider(
        "Forecast horizon (months)", options=[3, 6, 9, 12],
        value=st.session_state.get("global_horizon", 6),
        key="global_horizon", label_visibility="collapsed",
        help="Sets the forecast window for ALL modules")
    st.sidebar.markdown(
        ...
        unsafe_allow_html=True)
    st.sidebar.markdown("<div style='height:1px;background:rgba(100,116,139,0.15);margin-bottom:10px'></div>",
        unsafe_allow_html=True)
    PAGES = {
