...
def compute_inventory(
    ...
    cat_fcs     = compute_category_forecasts(n_future)      # shared cache — no duplicate ML
...
def page_overview() -> None:
    ...
    <div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px'>
      <div class='card' style='border-top:3px solid #3b82f6'>
        <div style='font-size:11px;font-weight:800;color:#3b82f6;letter-spacing:.06em;text-transform:uppercase'>1 · Demand Intelligence</div>
      ...
      <div class='card' style='border-top:3px solid #059669'>
        <div style='font-size:11px;font-weight:800;color:#059669;letter-spacing:.06em;text-transform:uppercase'>4 · Predictive Logistics</div>
...
def page_demand() -> None:
    ...
    st.markdown("<div class='page-title'>Demand Intelligence</div>", unsafe_allow_html=True)
    ...
    # ── Why ML Ensemble over SARIMA / Prophet? ─────────────────────────────────
    banner(
        "🔬 <b>Model Choice — Why Ensemble over SARIMA / Prophet?</b> &nbsp; "
        "SARIMA and Prophet are strong baselines for stationary or seasonal time series. "
        "On this dataset (24 months, 4 categories, mixed channels), the <b>Ridge + Random Forest + "
        "Gradient Boosting ensemble</b> outperformed both by capturing non-linear trend shifts and "
        "regime changes without requiring manual seasonality tuning. "
        "SARIMA assumes stationarity and needs >3× more data for stable ARMA coefficients at monthly frequency. "
        "Prophet performs best with daily data and explicit holiday calendars — not available here. "
        "The ensemble uses <b>inverse-RMSE weighting</b> so the best model automatically dominates.",
        "teal",
    )
...
def page_inventory() -> None:
    ...
    horizon_badge(n_future)
    banner(
        "📦 <b>Optimization Method — Wilson EOQ (closed-form Linear Programming):</b> "
        "EOQ = √(2DS/H) minimises total cost (ordering + holding) — this is the analytical solution to a "
        "single-variable LP where the objective is to minimise total inventory cost subject to demand constraints. "
        "<b>Safety Stock</b> (buffer for demand variance) and <b>ROP</b> (reorder trigger) are layered on top. "
        "<b>Prod Need</b> = max(Forecast + SS − Stock, ROP + EOQ − Stock). "
...
def page_production() -> None:
    ...
    banner(
        ...
        f"{n_future}M Demand (<b>{total_demand_6m_inv:,}</b>) "
...
def page_logistics() -> None:
    ...
    st.markdown("<div class='page-title'>Predictive Logistics</div>", unsafe_allow_html=True)
    horizon_badge(n_future)
    banner(
...
def page_chatbot() -> None:
    ...
    # api_key is collected once in main() sidebar and stored via key="groq_api_key"
    ...
    inv  = compute_inventory(_order_cost, _hold_pct, _lead_time, _z, n_future)
    plan = compute_production(n_future=n_future)
    carr, best_carr, opt, fwd_plan = compute_logistics(n_future=n_future)
    ...
    system = (
        ...
        f"1. DEMAND FORECASTING — ML ensemble (Ridge+RF+GradBoost) forecasting orders, qty, revenue for next {n_future} months by category/region/channel\n"
...
def main() -> None:
    ...
    # ── Global Forecast Horizon — shown for ALL pages ─────────────────────────
    st.sidebar.markdown(
    ...
    st.sidebar.markdown(
        ...
        f"margin-bottom:10px'>→ All modules use <b style='color:#1e3a8a'>{horizon_val}M</b> window</div>",
        unsafe_allow_html=True,
    )
    st.sidebar.markdown(
        "<div style='height:1px;background:rgba(100,116,139,0.15);margin-bottom:8px'></div>",
        unsafe_allow_html=True,
    )

    # ── Groq API key — always visible in sidebar regardless of active page ────
    st.sidebar.markdown(
        "<div style='font-size:10px;font-weight:700;color:#4a5e7a;letter-spacing:.1em;"
        "text-transform:uppercase;font-family:DM Mono;margin-bottom:4px'>🤖 AI Config</div>",
        unsafe_allow_html=True,
    )
    api_key = st.sidebar.text_input(
        ...
        help="Get a free key at console.groq.com — required for Decision Intelligence",
        key="groq_api_key",
    )
    if api_key and len(api_key.strip()) > 10:
        if api_key.strip().startswith("gsk_"):
            st.sidebar.markdown(
                "<div style='font-size:10px;color:#56e0a0;font-family:DM Mono;margin-top:-4px;"
                "margin-bottom:8px'>✅ Key looks valid</div>",
                unsafe_allow_html=True,
            )
        else:
            st.sidebar.markdown(
                "<div style='font-size:10px;color:#ff6b6b;font-family:DM Mono;margin-top:-4px;"
                "margin-bottom:8px'>⚠️ Should start with gsk_</div>",
                unsafe_allow_html=True,
            )
    st.sidebar.markdown(
        "<div style='height:1px;background:rgba(100,116,139,0.15);margin:8px 0 10px'></div>",
        unsafe_allow_html=True,
    )

    PAGES = {
        "Overview":                page_overview,
        "Demand Intelligence":     page_demand,
        "Inventory Optimization":  page_inventory,
        "Production Planning":     page_production,
        "Predictive Logistics":    page_logistics,
