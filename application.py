...
def CD() -> dict:
    return dict(
        ...
        font=dict(color="#334155",family="Inter,sans-serif",size=11),margin=dict(l=30, r=50, t=42, b=30)
    )
...
def leg() -> dict:
    return dict(
        bgcolor="rgba(255,255,255,0.95)",bordercolor="#E0E0E0",borderwidth=1,font=dict(color="#334155", size=10),
    )
...
def compute_inventory(
    ...
    sku_stats = (sku_monthly.groupby("SKU_ID")["Net_Qty"]
        .agg(hist_avg="mean",hist_std="std",peak_d="max").reset_index()
    )
...
def page_overview() -> None:
    ...
    <div style='background:linear-gradient(135deg,#0f172a,#1e3a8a,#2563eb);border-radius:18px;
      ...
      <div style='font-size:11px;font-family:DM Mono,monospace;color:#93c5fd;letter-spacing:.14em;
        ...
        Predictictive Logistics & AI Powered Demand to Delivery Optimization
      </div>
    </div>""", unsafe_allow_html=True)
            ...
            for status, clr in STATUS_CLR.items():
                ...
                fig_sc.add_trace(go.Scatter(
                    ...
                    marker=dict(size=bubble_sz, color=clr, opacity=0.82,
                        line=dict(color="#FFFFFF", width=1.5),sizemode="area", sizeref=2.0 * 60 / (40.0 ** 2), sizemin=6,
                    ),
                    ...
                    hovertemplate=(
                        ...
                        "Stock: %{x}<br>"
                        "ROP: %{y}<br>"
                        "6 months Demand: %{customdata[3]:,} units<br>"
...
def page_production() -> None:
    ...
    with pt2:
        ...
        wh_dist = (sku_plan.groupby("Target_Warehouse")
            .agg(SKUs=("SKU_ID", "count"), Units=("Prod_Need", "sum")).reset_index().sort_values("Units", ascending=False)
        )
        n_warehouses = len(wh_dist)
        wh_colors = ["#1e3a8a", "#2563eb", "#3b82f6", "#60a5fa", "#93c5fd", "#059669", "#10b981", "#34d399"][:n_warehouses]
