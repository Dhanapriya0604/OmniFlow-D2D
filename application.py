...
def compute_inventory(
    ...
    eoq = np.maximum(np.where(ann_d>0,np.sqrt(2*ann_d*order_cost / (uc*hold_pct)),10),1).astype(int)
...
def compute_logistics(
    ...
    opt["Potential_Saving"] = ((opt["Current_Avg_Cost"] - opt["Min_Avg_Cost"])* pt["Orders"]).round(0)
...
def page_overview() -> None:
    ...
    st.markdown("""
      <div style='font-size:38px;font-weight:900;color:white;letter-spacing:-.02em;
           text-transform:uppercase;line-height:1.1'>OmniFlow D2D
    ...
    <div style='font-size:16px;font-weight:900;margin-bottom:14px'>Data Scope</div>
    ...
    <div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px'>
      ...
      <div class='card' style='border-top:3px solid #8b5cf6'>
        ...
        <div style='font-size:11.5px;color:#475569;line-height:1.7'>
          Monthly production targets derived from the forecasted demand with safety stock excluding current stocks.
      ...
      <div class='card' style='border-top:3px solid #059669'>
        ...
        <div style='font-size:11.5px;color:#475569;line-height:1.7'>
          ...
          per region, delay hotspots by carrier with region, and projects forward shipping cost
    ...
    st.markdown("""
    <div class='about-section'>
    <div style='font-size:16px;font-weight:900;margin-bottom:14px'>Tech Stack</div>
    <div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px'>
      <div class='card'>
...
def page_inventory() -> None:
    ...
    kpi(c2,"🔴 Critical SKUs", n_crit, "coral", f"stock ≤ safety stock")
    kpi(c3,"🟡 Low Stock", n_low, "amber", f"stock < reorder point")
    ...
    with tab_alerts:
        ...
        if sv.empty:
            ...
            if not action.empty:
                ...
                tbl = action[[
                    "SKU_ID","Product_Name","Category","ABC","Status","Current_Stock","Demand_6M",
                    "Demand_Cover_Pct","ROP","EOQ","SS","Prod_Need"
                ]].copy()
                tbl.columns = [
                    "SKU","Product","Category","ABC","Status","Stock","6M Demand","Covers %",
...
def page_production() -> None:
    ...
    c1, c2, c3, c4, c5 = st.columns(5)
    kpi(c1, "Production Required",f"{plan['Production'].sum():,.0f}","amber","inventory-driven")
    kpi(c2, "6M Forecast Demand",f"{total_demand_6m_inv:,}","sky")
    kpi(c3, "Current Stock Total",f"{total_stock_inv:,}","sky","across all SKUs")
    kpi(c4, "Stock Gap (Prod Need)",f"{total_prod_need_inv:,}","coral")
    ...
    d3.columns = [
        "Month","Category","Cat Stock","6M Demand","Inv Prod Need",
...
def page_logistics() -> None:
    ...
    with t3:
        if fwd_plan.empty:
            ...
            with tc1:
                sec("Production to Shipment Plan")
                fig_fwd = go.Figure()
