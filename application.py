...
def page_demand():
    ...
    metric_opt = c1.selectbox("Metric",    ["Orders (#)","Quantity (Units)","Net Revenue (₹)"], key="demand_metric")
    level_opt  = c2.selectbox("Breakdown", ["Overall","Category","Region","Sales Channel"], key="demand_level")
    horizon    = c3.slider("Months ahead", 3, 12, 6, key="demand_horizon")

    col_map = {"Orders (#)":"Order_ID","Quantity (Units)":"Net_Qty","Net Revenue (₹)":"Net_Revenue"}
    col = col_map.get(metric_opt, "Order_ID")
...
def page_logistics():
    ...
    with t5:
        ...
        plot_col = met_col_map.get(met, "Revenue")
        y_vals = rs[plot_col]
