...
def production_planning(forecast_df, inventory_df, manufacturing_df): 
    ...
    inv = (
        inventory_df.groupby("product_id", as_index=False).agg(current_stock=("on_hand_qty", "sum"))
    )
...
def auto_production_schedule(prod_df):
    ...
    if len(schedule_rows) == 0:
        return pd.DataFrame(columns=["date", "product_id", "production_qty"])
    return pd.DataFrame(schedule_rows)
def allocate_production_lines(schedule_df):
    if schedule_df.empty:
        return pd.DataFrame(columns=["date","line","product_id","production_qty"])
    schedule_rows = []
...
PRODUCTION_DATA_DICTIONARY = pd.DataFrame({
    "Column": [
        "product_id","avg_daily_demand","planning_demand","current_stock",
        "production_required","batch_size","production_batches","daily_capacity",
        "days_required","production_status"
    ],
    "Description": [
        "Unique product identifier","Average forecasted daily demand","14-day planning demand",
        "Current inventory level","Units required to meet demand","Production batch size",
        "Number of batches needed","Daily manufacturing capacity",
        "Days required to complete production", "Production decision status"
    ]
})
...
def production_planning_page():
    ...
    with tab2:
        ...
        view_mode = st.radio("Production View",["All Products", "Single Product"],horizontal=True)    
        ...
        fig_ds = px.bar(view_prod_df, x="product_id", y=["planning_demand", "current_stock"],
        ...
        st.plotly_chart(
            px.bar(view_prod_df, x="product_id", y="production_required",
                   color="production_status", template="plotly_white"),
            use_container_width=True
        )
        st.markdown('<div class="section-title">Production Plan Output</div>', unsafe_allow_html=True)
        st.dataframe(
            view_prod_df[["product_id","production_required",
                "daily_capacity","days_required","production_status"
            ]], use_container_width=True
        )      
        c1, c2, c3 = st.columns(3)
        with c1:
            st.download_button("⬇ Production Plan",
                prod_df.to_csv(index=False),"production_plan.csv"
            )     
        with c2:
            st.download_button("⬇ Production Schedule",
                schedule_df.to_csv(index=False),"production_schedule.csv"
            )     
        with c3:
            st.download_button("⬇ Line Allocation",
                line_schedule_df.to_csv(index=False),"line_allocation.csv"
            )
