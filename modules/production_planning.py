...
def production_planning(forecast_df, inventory_df, manufacturing_df):  
    ...
    demand = (
        forecast_df.groupby("product_id").apply(lambda x: x.head(14))
        .reset_index(drop=True).groupby("product_id")["forecast"]
    ...
    df["production_status"] = np.where(
        df["production_required"] > 0, "⚠ Production Needed", "✅ Stock Sufficient"
    )  
...
PRODUCTION_DATA_DICTIONARY = pd.DataFrame({
    "Column": [
        "product_id","avg_daily_demand","planning_demand",
        "current_stock","production_required","batch_size",
        "production_batches","daily_capacity",
        "days_required","production_status"
    ],
    "Description": [
        "Unique product identifier","Average forecasted daily demand",
        "14-day planning demand","Current inventory level",
        "Units required to meet demand","Production batch size",
...
def production_planning_page():
    ...
    with tab2:
        ...
        if prod_df["backlog"].sum() == 0:
            st.info("Production capacity is sufficient. No backlog detected.")
        else:
            st.warning("Backlog detected. Production capacity insufficient for demand.")
        ...
        st.plotly_chart(
            px.bar(view_prod_df,
                   x="product_id", y="production_required",
        ...
        st.dataframe(
            view_prod_df[[
                "product_id","production_required","daily_capacity","days_required","production_status"
            ]],
            use_container_width=True
        )      
        c1, c2, c3 = st.columns(3)
