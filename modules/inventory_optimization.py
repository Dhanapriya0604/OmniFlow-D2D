...
DATA_DICTIONARY = pd.DataFrame({
    ...
    "Description": [
        ...
        "Buffer stock to handle demand uncertainty",
...
def inventory_optimization(forecast_df, inventory_df):
    ...
    df["EOQ"] = np.minimum(
        df["EOQ"],
        df["avg_daily_demand"] * 60   # max 2 months demand
    )
    ...
    df["safety_stock"] = np.maximum(
        df["safety_stock"],
        df["avg_daily_demand"] * 2
    )

    df["reorder_point"] = np.ceil(
        df["avg_daily_demand"] * lead_time_days +
        df["safety_stock"]
    )
    df["stockout_risk"] = (
...
def inventory_optimization_page():
    ...
    with tab2:
        ...
        # ======================================================================================
        # LOAD FORECASTS (SESSION FIRST, FILE FALLBACK)
        # ======================================================================================
